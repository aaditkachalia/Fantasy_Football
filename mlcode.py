# -*- coding: utf-8 -*-
"""
Created on Mon Apr 16 21:53:36 2018
...
level_cluster = pd.read_csv('level_cluster.csv')
level7_d = pd.read_csv('level7_d.csv')
...
X_gk_main = df[(df['Club_Position'] == 'GK')]
X_gk = X_gk_main[['Weak_foot','Skill_Moves','Ball_Control','Dribbling','Marking','Sliding_Tackle','Standing_Tackle','Aggression','Reactions','Attacking_Position','Interceptions','Vision','Composure','Crossing','Short_Pass','Long_Pass','Acceleration','Speed','Stamina','Strength','Balance','Agility','Jumping','Heading','Shot_Power','Finishing','Long_Shots','Curve','Freekick_Accuracy','Penalties','Volleys','GK_Positioning','GK_Diving','GK_Kicking','GK_Handling','GK_Reflexes','Work_Rate_Points', 'Age_Points', 'BMI']]
X_scaled_gk = MinMaxScaler(feature_range=(0, 10)).fit_transform(X_gk)
...
X_r_gk = pca.fit(X_scaled_gk).transform(X_scaled_gk)
kmeans = KMeans(init='k-means++', n_clusters=8, n_init=10)
kmeans.fit(X_r_gk)
label_color_gk = [matplotlib.cm.spectral(float(i) /10) for i in kmeans.labels_]
X_gk_main['Cluster'] = kmeans.labels_
...
ss = X_def_main[(X_def_main['Cluster'] == 7)].sort_values('Rating', ascending = False)[:10]
...
ss = X_att_main[(X_att_main['Cluster'] == 5)].sort_values('Rating', ascending = False)[:10]
sns.set(style="white", context="talk")
...
check1 = X_gk_main.groupby('Cluster', as_index=False).agg({'Rating': 'mean'})
check2 = X_gk_main.groupby('Cluster', as_index=False).agg({'Rating': 'std'})
check1['Mean+Std'] = round(check1['Rating'] + check2['Rating'])
check1

ss = X_gk_main[(X_gk_main['Cluster'] == 4)].sort_values('Rating', ascending = False)[:10]
sns.set(style="white", context="talk")
...
ss = X_gk_main[(X_gk_main['Cluster'] == 2)].sort_values('Rating', ascending = False)[:10]
sns.set(style="white", context="talk")
